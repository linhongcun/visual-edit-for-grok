 ...
 * Inspired by mature terminal UX + xterm.js patterns — reimplemented (no source copy).
 */
...
function clampMinimumContrastRatio(
  ...
  const n = typeof value === "number" ? value : Number(value);
  if (!Number.isFinite(n)) return fallback;
  // Keep one decimal for AA (4.5); round extremes to ints when whole
...
function planWebglContextLoss(input = {}) {
  ...
  // Already used all retries → stay on canvas after dispose
